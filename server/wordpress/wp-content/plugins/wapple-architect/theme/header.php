...
// Use Google Analyticator tracking ID
$ga_use_analyticator = get_option('architect_analytics_use_analyticator');

echo '<pre>$ga_use_analyticator:'.$ga_use_analyticator.'</pre>';

if(function_exists('is_plugin_active') AND is_plugin_active('google-analyticator/google-analyticator.php'))
{
	$hasGoogleAnalyticator = true;
} else if(in_array('google-analyticator/google-analyticator.php', (array) get_option('active_plugins', array())))
{
	$hasGoogleAnalyticator = true;
} else
{
	$hasGoogleAnalyticator = false;
}

if($ga_use_analyticator === false AND $hasGoogleAnalyticator AND !get_option('architect_analytics_google_tracking_code'))
{
	// No set - but assume we want to use it
	if(get_option('ga_uid') AND get_option('ga_status') == 'enabled')
	{
		$ga_use_analyticator = true;
	}
}

if($ga_use_analyticator AND $hasGoogleAnalyticator AND get_option('ga_status') == 'enabled')
{
	$useGA = true;
	$useGATrackingId = get_option('ga_uid');
} else
{
	// Do we want to use analytics setting specified in options
	$useGATrackingId = get_option('architect_analytics_google_tracking_code');
	if($useGATrackingId)
	{
		$useGA = true;
	}
...
// Display tracking code if we have specified one
if($useGA)
{
	$string .= '<googleAnalyticsTrackingCode>'.$useGATrackingId.'</googleAnalyticsTrackingCode>';
}
...
// Canonical link for Google SEO goodness
if(is_home())
{
	$string .= '<canonical_link>'.get_bloginfo("siteurl").'</canonical_link>';
} else
{
	$string .= '<canonical_link>'.get_permalink().'</canonical_link>';
}
